...
@ControllerAdvice
public class ResponseEntityGlobalExceptionHandler extends ResponseEntityExceptionHandler {

    //	private JSONObject claims;
    //
    //	@ModelAttribute("claims")
    //	protected JSONObject extractUserFromJWT(HttpServletRequest request) throws ParseException,
    ...
    //
    //			String token = request.getHeader("Authorization").split(" ")[1]; // get jwt from
    //			header
    ...
    //
    //			List<String> roles = (List<String>) Optional.ofNullable((claimsObj.get("role")))
    ...
    //				message = "The phone number has already existed or registered by another
    //				user.";
    //				break;
    ...
    protected ResponseEntity<Object> handleEmptyResultDataAccessException(Exception ex,
                                                                          WebRequest request) {
        ex.printStackTrace();
    ...
    protected ResponseEntity<Object> handleBadRequestException(Exception ex, WebRequest request) {
        ...
        apiError.setPath(request.getDescription(true).split(";")[0].split("=")[1]);
        apiError.setMessage(
                !apiError.getError().isEmpty() ? apiError.getError() : "Invalid Request Type!");
    ...
    protected ResponseEntity<Object> handleAuthenticationApiException(Exception ex,
                                                                      WebRequest request) {
        ex.printStackTrace();
    ...
    protected ResponseEntity<Object> handleInternalAuthenticationServiceException(Exception ex,
                                                                                  WebRequest request) {
        ex.printStackTrace();
    ...
    @ExceptionHandler(
            {AccessDeniedException.class, AccessForbidden.class, AuthenticationException.class})
    ...
    //	@ExceptionHandler({ TransactionSystemException.class, DataIntegrityViolationException
    //	.class })
    ...
    //
    //				constraintViolations = ((ConstraintViolationException) cause)
    //				.getConstraintViolations();
    //				String errorMessage = null;
    //
    //				for (Iterator<ConstraintViolation<?>> it = constraintViolations.iterator(); it
    //				.hasNext();) {
    ...
    //			apiError
    //					.setMessage(((DataIntegrityViolationException) ex).getRootCause()
    //					.getLocalizedMessage());
    //		}
    //
    //		return new ResponseEntity<Object>(apiError, new HttpHeaders(), apiError.getStatus());
    ...
    //	protected ResponseEntity<Object> hanlderCustomExceptions(Exception ex, WebRequest
    //	request) {
    //
    //		ex.printStackTrace();
    //
