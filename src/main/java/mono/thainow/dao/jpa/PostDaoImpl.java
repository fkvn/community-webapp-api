...
public class PostDaoImpl implements PostDao {
    ...
    public List<Post> findPostsByProfile(Profile profile) {
        return entityManager.createQuery("from Post where owner =: owner", Post.class)
                .setParameter("owner", profile).getResultList();
    }
    ...
    //		}
    //
    //		List<PostStatus> validPostStatus = ownerRequest ? Arrays.asList(PostStatus.AVAILABLE,
	//		PostStatus.PRIVATE)
    //				: Arrays.asList(PostStatus.AVAILABLE);
    //
    //		return entityManager.createQuery(sql, Post.class).setParameter("status",
	//		validPostStatus)
    //				.setParameter("owner", postOwner).setFirstResult((page - 1) * limit)
	//				.setMaxResults(limit)
    //				.getResultList();
    //	}
    ...
    public Post findPostById(long id) {
        ...
        if (post == null) throw new NoResultException();
    ...
    public Post findValidPost(Long postId, PostType type) {
        ...
        return entityManager.createQuery(sql, Post.class)
                .setParameter("status", Arrays.asList(PostStatus.PUBLIC, PostStatus.PRIVATE))
    ...
    //		}
    //
    //		return entityManager.createQuery(sql, Post.class).setParameter("entity", entity)
	//		.getSingleResult();
    //	}
