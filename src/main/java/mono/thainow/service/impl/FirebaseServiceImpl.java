...
public class FirebaseServiceImpl implements FirebaseService {
    ...
    public String uploadFile(File file, String fileName, String fileType) {
        ...
        String DOWNLOAD_URL =
                "https://firebasestorage.googleapis.com/v0/b/mono-thainow.appspot" +
						".com/o/thainow-service-worker%%2F%s?alt=media";
        //		String DOWNLOAD_URL = "/cdn/image/thainow-service-worker%%2F%s?alt=media";


        BlobId blobId = BlobId.of(FirebaseConfig.getSTORAGE_BUCKET_URL(),
                "thainow-service-worker/" + fileName);
        BlobInfo blobInfo = BlobInfo.newBuilder(blobId).setContentType(fileType).build();

        try {

            //			persist to firebase bucket
            fbStorageClient.getBucket().getStorage()
