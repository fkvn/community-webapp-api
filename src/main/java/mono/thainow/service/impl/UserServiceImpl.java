...
public class UserServiceImpl implements UserService {
    ...
    public Optional<User> findUserByPhone(String phone, String region) {
        return userRepository.findByPhoneAndPhoneRegionAndStatusNot(phone, region,
                UserStatus.DELETED);
    }
    ...
    public Optional<User> findActiveUserById(Long id) {
        ...
        if (user.isPresent()) {
            if (user.get().getStatus().equals(UserStatus.DISABLED)) throw new BadRequest(
                    "This account is disabled. Please contact the administrators!");
    ...
    public User fetchUserFromUserRequest(User user, UserRequest request) {
        ...
        if (!email.isBlank() && user.getEmail() != null && !user.getEmail().equals(email)) {
            Assert.isTrue(user.getProvider().equals(UserProvider.THAINOW),
                    "This profile email is managed by " + user.getProvider() + " account.");
            Assert.isTrue(!emailService.isEmailExisting(email), "Email has already been taken.");
        }
        user.setEmail(email);

        //		set phone
        if (!phone.isBlank() && user.getPhone() != null && !user.getPhone().equals(phone)) {
            String phoneRegion = Optional.ofNullable(request.getPhoneRegion()).orElse("US").trim();
        ...
        String username = Optional.ofNullable(request.getUsername())
                .orElse((firstname.isBlank() && lastname.isBlank() ? email :
        ...
        Boolean isDescriptionPublic =
                Optional.ofNullable(request.getIsDescriptionPublic()).orElse(false);
    ...
    //		if (address != null)
    //			if (!address.isBlank())
    //				user.setLocation(locationService.findLocationByPlaceidOrAddress(placeid,
    //				address));
    //			else
    //				user.setLocation(null);
    //
    ////		public location
    //		Boolean isLocationPublic = Optional.ofNullable(request.getIsLocationPublic()).orElse
    //		(null);
    ...
    //
    ////		verify email
    //		Boolean isEmailVerified = Optional.ofNullable(request.getIsEmailVerified()).orElse
    //		(null);
    ...
    //
    ////		verify phone
    //		Boolean isPhoneVerified = Optional.ofNullable(request.getIsPhoneVerified()).orElse
    //		(null);
